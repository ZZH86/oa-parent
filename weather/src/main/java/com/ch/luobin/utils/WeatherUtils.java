...
public class WeatherUtils {
     ...
     */
    public static Forecast2 getWeather() throws JsonProcessingException {
        Map<String,String> map = new HashMap<>();
        map.put("district_id", city);
        map.put("key",chpKet);
        RestTemplate restTemplate = new AppConfig().restTemplate();
        String res = restTemplate.getForEntity( "https://apis.tianapi.com/tianqi/index?city={district_id}&key={key}&type=1", String.class, map).getBody();
        ...
        Forecast2 forecast = new Forecast2();

        if (StringUtils.isNoneBlank(alarmlist)){
            List<Forecast2> jsonList = JSONArray.parseArray(alarmlist, Forecast2.class);
